...
Class model_usuarios extends model{
    ...
    public function confere_login($usuario, $senha){
    	$db = new mysql();
		$exec = $db->executar("SELECT codigo, usuario, senha FROM ".$this->tab_usuario." WHERE usuario='$usuario' ");
		if($exec->num_rows == 1){
			...
			if (security::verify_password($senha, $data->senha)) {
				if (security::needs_rehash($data->senha)) {
					...
					$db->alterar($this->tab_usuario, array("senha" => $nova_senha), "codigo='" . $data->codigo . "'");
				}
				return $data;
			}
		}
