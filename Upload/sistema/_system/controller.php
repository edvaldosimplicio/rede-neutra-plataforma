...
class controller extends system {
	...
	protected function autenticacao(){
		...
		if( isset($_SESSION['adm_sessao']) AND isset($_SESSION['adm_acesso']) AND isset($_SESSION['adm_cod_usuario']) ){
			...
			// SEGURANÇA: Verifica se a sessão pertence ao mesmo IP e navegador do login
			// Recalcula o hash e compara com o valor armazenado no login
			$hash_esperado = md5(TOKEN1.$_SERVER['REMOTE_ADDR'].$_SERVER['HTTP_USER_AGENT']);
			if($_SESSION['adm_acesso'] != $hash_esperado){
				session_destroy();
				$this->irpara( DOMINIO );
			}
